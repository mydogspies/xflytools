...
/**
 * The actual implementation of the command pattern for the Nav 1 standby radio frequency in Xplane.
 ...
 * @since 0.4.0
 */
public class RadiosNav1StdbyFreqSend implements OutCommand {

    private static final Logger log = LoggerFactory.getLogger(RadiosNav1StdbyFreqSend.class);
    ...
    public void execute() {
        ...
        String val = nav1Stby.getText();
        if (controller.matchesFreqFormat(val, 2)) {
            val = controller.formatFreqToSend(val, 2);
            main_controller.sendToXplane("set", "nav1_stdby_freq", val);
            log.trace("addToField(): Nav1 standby set to " + val + " in Xplane.");
        }
    }
}
