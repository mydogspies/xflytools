...
/**
 * The actual implementation of the command pattern for the Adf 1 standby radio frequency in Xplane.
 ...
 * @since 0.4.0
 */
public class RadiosAdf1StdbyFreqSend implements OutCommand {

    private static final Logger log = LoggerFactory.getLogger(RadiosAdf1StdbyFreqSend.class);
    ...
    public void execute() {
        ...
        String val = adf1Stby.getText();
        if (controller.matchesFreqFormat(val, 0)) {
            val = controller.formatFreqToSend(val, 0);
            main_controller.sendToXplane("set", "adf1_stdby_freq", val);
            log.trace("addToField(): Adf1 stand-by set to " + val + " in Xplane.");
        }
    }
}
