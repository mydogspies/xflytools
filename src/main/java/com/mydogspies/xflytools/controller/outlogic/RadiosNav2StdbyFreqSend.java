...
/**
 * The actual implementation of the command pattern for the Nav 2 standby radio frequency in Xplane.
 ...
 * @since 0.4.0
 */
public class RadiosNav2StdbyFreqSend implements OutCommand {

    private static final Logger log = LoggerFactory.getLogger(RadiosNav2StdbyFreqSend.class);
    ...
    public void execute() {
        ...
        String val = nav2Stby.getText();
        if (controller.matchesFreqFormat(val, 2)) {
            val = controller.formatFreqToSend(val, 2);
            main_controller.sendToXplane("set", "nav2_stdby_freq", val);
            log.trace("addToField(): Nav2 standby set to " + val + " in Xplane.");
        }
    }
}
